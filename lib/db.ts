...
export function computeDailyChars(writings: WritingEntry[]): number {
  ...
  return writings
    .filter(w => w.date === today || w.createdAt?.slice(0, 10) === today)
    .reduce((sum, w) => sum + w.text.length, 0);
}

const GAMIFICATION_KEY = 'wc_gamification';
export interface GamificationStore { dailyGoal: number; }
const GAMIFICATION_DEFAULTS: GamificationStore = { dailyGoal: 300 };

export function loadGamification(): GamificationStore {
  if (typeof window === 'undefined') return { ...GAMIFICATION_DEFAULTS };
  try {
    const s = JSON.parse(localStorage.getItem(GAMIFICATION_KEY) || 'null');
    return { ...GAMIFICATION_DEFAULTS, ...s };
  } catch { return { ...GAMIFICATION_DEFAULTS }; }
}
export function saveGamification(g: GamificationStore) {
